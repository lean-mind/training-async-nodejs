...
<CodeSurfer>
  ```js title="Eventos"
  export function readFileFromStream(readableStream) {
    const data = [];
...
<Notes>
  - Event Emitter en Node.js, Custom Event en Navegador
  - La gestión de asincronía por eventos marcó la diferencia con Node.js.
  - En su momento estuve en un equipo en el que creamos nuestro propio Promise con el Event Emitter, ya que en ese entonces todavía Promise no era estable.
</Notes>
...
<Appear>
  ...
  - No tengas miedo de usar promisify para convertir callbacks en promesas.
